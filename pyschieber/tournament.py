...
class Tournament:
    ...
    def play(
        ...
        while not end:
            benchmark_seed = set_benchmark_enabled(
                benchmark_enabled, round_counter, benchmark_seed
            )
            game = Game(
                teams=self.teams,
                point_limit=self.point_limit,
                use_counting_factor=use_counting_factor,
            )
            ...
            logger.info("-" * 200)
                ...
                start_player_index=((len(self.games) - 1) % 4),
                whole_rounds=whole_rounds,
            ...
            logger.info(
                f"Points: Team 1: {self.teams[0].points} , Team 2: {self.teams[1].points}. \n"
            )
            round_counter += 1
        ...
        logger.info(
            f" Team 0: {self.teams[0].points} vs {self.teams[1].points} Team 1.\n"
        )
...
def set_benchmark_enabled(
    benchmark_enabled: bool, round_counter: int, benchmark_seed: float
) -> float:
    """_summary_
