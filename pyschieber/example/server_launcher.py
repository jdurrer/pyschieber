...
def launch(server_address, session_name):
    opponents = [
        ServerPlayer(
            pyschieber_bot=GreedyPlayer(name="Greedy1"),
            server_address=server_address,
            chosen_team_index=1,
            session_name=session_name,
        ),
        ServerPlayer(
            pyschieber_bot=GreedyPlayer(name="Greedy2"),
            server_address=server_address,
            chosen_team_index=1,
            session_name=session_name,
        ),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="SchieberJassBot",
    )
    parser.add_argument(
        "-a",
        "--server_address",
        dest="server_address",
        help="Default: ws://127.0.0.1:3000",
    )
    parser.add_argument(
        "-s", "--session_name", dest="session_name", help="Default: test"
    )
