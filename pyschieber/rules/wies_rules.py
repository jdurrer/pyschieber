...
def vgl_weis(max_weis, total_weis, weis_cards, trumpf_value, start_player):
    ...
    # höchste Karte
        ...
        if trumpf_value == 2:  # une_ufe
            ...
            if min(highcard[0, 0], highcard[0, 2]) < min(
                highcard[0, 1], highcard[0, 3]
            ):
                # print('team0 tiefer')
                weis = total_weis[0, 0] + total_weis[0, 2]
                teamnbr = 0
            ...
            elif min(highcard[0, 0], highcard[0, 2]) > min(
                highcard[0, 1], highcard[0, 3]
            ):
                # print('team1 tiefer')
                weis = total_weis[0, 1] + total_weis[0, 3]
                ...
                for i in range(4):
                    ...
                    if highcard[0, player] == decisionCard:
                        weis = (
                            total_weis[0, player % 2] + total_weis[0, (player % 2) + 2]
                        )
                        teamnbr = player % 2
            ...
            if max(highcard[0, 0], highcard[0, 2]) > max(
                highcard[0, 1], highcard[0, 3]
            ):
                # print('maxweis t0')
                weis = total_weis[0, 0] + total_weis[0, 2]
            ...
            elif max(highcard[0, 0], highcard[0, 2]) < max(
                highcard[0, 1], highcard[0, 3]
            ):
                # print('maxweis t1')
                weis = total_weis[0, 1] + total_weis[0, 3]
