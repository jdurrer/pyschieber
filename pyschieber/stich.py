...
from typing import Literal, TypedDict  # Required for Python < 3.9
...
def played_cards_dict(played_card: PlayedCard) -> PlayedCardsDict:
    return {"player_id": played_card.player.id, "card": str(played_card.card)}
...
def stich_dict(stich: Stich) -> StichDict:
    return {
        ...
        "played_cards": [
            played_cards_dict(played_card) for played_card in stich.played_cards
        ],
    }
