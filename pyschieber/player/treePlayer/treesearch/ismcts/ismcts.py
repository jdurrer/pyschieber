...
class _InvalidAction:
    """Unique sentinel — never a real item."""
...
class ChildInfo:
    ...
    def value(self) -> float:
        """Compute the mean return value for this child node.

        This method divides the accumulated return by the number of visits
        to obtain the average payoff observed for this child.

        Returns:
            float: The average return value over all visits to this child node.
        """
...
class ISMCTSNode:
    ...
    i.e. possible actions are represented in a list and the integer
    points to the action in the list.

    """

    def __init__(self) -> None:
        self.child_info: dict[Card, ChildInfo] = {}
        self.total_visits: int = 0
        self.prior_map: dict[Card, float] = {}
...
class ISMCTSBot:
    ...
    Args:
        ...
        uct_c: The exploration constant used in the tree policy. For MCTS usuall sqrt(2).
            For ISMCTS usually 0.7. However, here we do not use 0 loss and 1 win but points -> 257 * 0.7
    ...
    def __init__(
        ...
        evaluator: Evaluator,
        max_simulations: int,
        informationset: CardDistributionSolver,
        uct_c: float = 180,
        ...
        child_selection_policy: ChildSelectionPolicy = ChildSelectionPolicy.UCT,
    ) -> None:
        ...
        self._nodes: dict[tuple[int, int], ISMCTSNode] = {}
        self._node_pool: list[ISMCTSNode] = []
        self._root_samples: list[GameState] = []
    ...
    def random_number(self) -> float:
        """Generate a single random floating-point number from the internal RNG.

        This method delegates to the bot's configured NumPy random state to
        produce a uniformly distributed value in the half-open interval [0.0, 1.0).

        Returns:
            float: A uniformly distributed random number in the range [0.0, 1.0).
        """
        return self._random_state.uniform()

    def reset(self) -> None:
        """Clear all internal IS-MCTS search state for a fresh search.

        This method discards any previously stored tree nodes, node pool
        entries, and cached root samples so that subsequent searches start
        from an empty tree.
    ...
    def get_state_key(self, state: GameState) -> tuple[int, int]:
        """Return a key representing the current information state.

        Args:
            state (GameState): The game state from which to derive the key.

        Returns:
            tuple[int, int]: current player index as integer, information about state as integer.
        """
        return state.current_player(), state.information_state_string()

    def normalized_visited_policy(self, node: ISMCTSNode) -> list[tuple[Card, float]]:
        ...
        Args:
            state (SearchState): The current game state associated with the node.
            node (ISMCTSNode): The ISMCTS node whose child visit statistics define the policy.

        Returns:
            list[tuple[Card, float]]: policy - A list of (action, probability) pairs representing a normalized
    ...
    def max_visit_policy(self, node: ISMCTSNode) -> list[tuple[Card, float]]:
        ...
        Returns:
            list[tuple[Card, float]]: policy - A list of (action, probability) pairs where
        ...
        for child in node.child_info.values():
            if child.visits == max_visits:
                ...
                count = 1
        return [
            (action, 1.0 / count if child.visits == max_visits else 0.0)
            for action, child in node.child_info.items()
        ]

    def max_value_policy(self, node: ISMCTSNode) -> list[tuple[Card, float]]:
        ...
        Returns:
            list[tuple[Card, float]]: A list of (action, probability) pairs where
        ...
        for child in node.child_info.values():
            if child.value() == max_value:
                ...
                count = 1
        return [
            (action, 1.0 / count if child.value() == max_value else 0.0)
            for action, child in node.child_info.items()
        ]

    def _compute_base_policy(self, node: ISMCTSNode) -> list[tuple[Card, float]]:
        ...
        Returns:
            list[tuple[Card, float]]: policy - A list of (action, probability) pairs representing the base policy.
        """
    ...
    def _pad_policy_with_unexpanded_actions(
        self, state: GameState, node: ISMCTSNode, policy: list[tuple[Card, float]]
        ...
        Args:
            state (GameState): The game state whose legal actions must be covered by the policy.
            node (ISMCTSNode): The ISMCTS node associated with the state.
            policy (list[tuple[Card, float]]): The already computed base policy to extend.

        Returns:
            list[tuple[Card, float]]: The extended policy including missing legal actions with zero probability.
        """
    ...
    def get_final_policy(
        self, state: GameState, node: ISMCTSNode
    ) -> list[tuple[Card, float]]:
        ...
        Args:
            state (GameState): The current game state whose legal actions define the policy support.
            node (ISMCTSNode): The search node whose visit and value statistics form the base policy.

        Returns:
            list[tuple[Card, float]]: A list of (action, probability) pairs representing the final policy.
        """
        assert node
    ...
    def resample_from_infostate(self, state: GameState) -> GameState:
        """Resample a determinizated state consistent with the current information state.

        Args:
            state (SearchState): The game state whose information state is used as
                the basis for generating a new determinizated state.

        Returns:
            SearchState: A newly sampled state compatible with the current player's
            information state in the provided `state`.
        """
        return state.resample_from_infostate(self.informationset)

    def sample_root_state(self, state: GameState) -> GameState:
        ...
        Args:
            state (SearchState): The original game state whose information state
        ...
        Returns:
            SearchState: A determinizated root state consistent with the given
        ...
        if self._max_world_samples == UNLIMITED_NUM_WORLD_SAMPLES:
            return self.resample_from_infostate(state).clone()
            ...
            raise ValueError("Case not handled (badly set max_world_samples..?)")

    def create_new_node(self, state: GameState) -> ISMCTSNode:
        ...
        Args:
            state (SearchState): The game state for which to create a node.
    ...
    def lookup_node(self, state: GameState) -> ISMCTSNode | None:
        ...
        Args:
            state (SearchState): The game state whose associated node is being requested.
    ...
    def lookup_or_create_node(self, state: GameState) -> ISMCTSNode:
        ...
        Args:
            state (SearchState): The game state whose search node is requested.
    ...
    def filter_illegals(
        self, node: ISMCTSNode, legal_actions: list[Card]
    ) -> ISMCTSNode:
    ...
    def repopulate_prior_map_inconsistent_action_sets(
        self, state: GameState, node: ISMCTSNode
    ) -> None:
        ...
        Args:
            state (GameState): The game state providing the current action priors.
    ...
    def expand_if_necessary(
        self, state: GameState, node: ISMCTSNode, action: Card
    ) -> None:
        ...
        Args:
            state (GameState): The current game state used to refresh priors if needed.
            node (ISMCTSNode): The search node at which the child action may be expanded.
            action (Card): The action identifier that should be present in the node's children.
        """
        if action not in node.child_info:
    ...
    def _action_value(self, node: ISMCTSNode, child: ChildInfo) -> float:
        ...
        if self._child_selection_policy == ChildSelectionPolicy.UCT:
            ...
            raise ValueError("Child selection policy unrecognized.")
        return action_value

    def _select_candidate_actions(self, node: ISMCTSNode) -> list[Card]:
        ...
        Returns:
            list[Card]: A list of action identifiers that form the candidate set.
        """
    ...
    def select_action(self, node: ISMCTSNode) -> Card:
        ...
        Returns:
            Card: The chosen action identifier from the candidate action set.
        """
        candidates = self._select_candidate_actions(node)
        assert len(candidates) >= 1
        return candidates[self._random_state.randint(len(candidates))]

    def select_action_tree_policy(
        self, state: GameState, node: ISMCTSNode, legal_actions: list[Card]
    ) -> Card:
        ...
        Args:
            state (GameState): The current game state from which the selection is made.
            node (ISMCTSNode): The ISMCTS node representing the current information set.
            legal_actions (list[Card]): The list of legal actions available in the current state.

        Returns:
            Card: The chosen action as a Card object.
        """
        ...
        action = legal_actions[
            self._random_state.randint(len(legal_actions))
        ]  # prior? (not my comment)
    ...
    def check_expand(
        ...
        Args:
            ...
            legal_actions (list[Card]): The list of actions that are legal in the current state.

        Returns:
            Card: The identifier of an unexpanded legal action to add as a child,
            or pyspiel.INVALID_ACTION (-1) if no expansion should occur.
        """
        if not self._allow_inconsistent_action_sets and len(node.child_info) == len(
            legal_actions
        ):
            return _InvalidAction()
        legal_actions_copy = copy.deepcopy(legal_actions)
        parsed_actions = list(range(len(legal_actions_copy)))
        self._random_state.shuffle(parsed_actions)
        for action in parsed_actions:
            if legal_actions_copy[action] not in node.child_info:
                return legal_actions_copy[action]

        return _InvalidAction()

    def run_simulation(self, state: GameState) -> NDArray[np.float64]:
        """Recursively simulate a single IS-MCTS rollout from the given game state.

        This method traverses terminal, chance, and decision states, expanding
        nodes as needed, updating visit and return statistics, and finally
        returning the payoff vector for all players.

        Args:
            state (GameState): The current game state from which the rollout
                is simulated.

        Returns:
            list[float]: A list of numeric payoffs corresponding to each player
            at the end of the simulated trajectory.
        """
        if state.is_terminal():
            return state.returns()

        if state.is_chance_node():
            action_list, prob_list = zip(*state.chance_outcomes())  # type: ignore
            chance_action = self._random_state.choice(action_list, p=prob_list)  # type: ignore
        ...
        chosen_action = self.check_expand(
            node, legal_actions
        )  # add one children at a time?
        if not isinstance(chosen_action, _InvalidAction):
            self.expand_if_necessary(state, node, chosen_action)
        else:
            chosen_action = self.select_action_tree_policy(state, node, legal_actions)

        assert not isinstance(chosen_action, _InvalidAction)

        node.total_visits += 1
        node.child_info[chosen_action].visits += 1
        state.apply_action(chosen_action)
        returns = self.run_simulation(state)
        node.child_info[chosen_action].return_sum += returns[current_player]
        return returns

    def run_search(self, state: GameState) -> list[tuple[Card, float]]:
        ...
        Args:
            state (GameState): The imperfect-information game state for which
                an IS-MCTS-based action policy is computed.

        Returns:
            list[tuple[Card, float]]: A list of (action, probability) pairs
        ...
        for _ in range(self._max_simulations):
            ...
            assert sampled_root_state is not state
    ...
    def step(self, state: GameState) -> Any:
        ...
        Args:
            state (GameState): The current game state from which the simulation is executed.
        ...
        """
        if state.is_chance_node():
            raise ValueError("State is a chance node, cannot compute policy.")
        action_list, prob_list = zip(*self.run_search(state))
        return self._random_state.choice(action_list, p=prob_list)

    def get_policy(self, state: GameState) -> list[tuple[Card, float]]:
        ...
        Args:
            state (GameState): The current game state for which an action
                policy is requested.
    ...
    def step_with_policy(
    ...
    ) -> tuple[list[tuple[Card, float]], Any]:
        ...
        Args:
            state (GameState): The current game state for which a policy and
                sampled action are requested.

        Returns:
            tuple[list[tuple[Card, float]], Any]: A tuple containing the list of
