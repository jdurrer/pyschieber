...
from pyschieber.stich import PlayedCard, played_cards_dict, stich_dict
...
class TreeSearch:
    ...
    def root(self):
        ...
        npc_cards[self.cc.partner_id] = BitwiseToCards(
            UnicatesPerPlayer[self.cc.partner_id]
        )
        npc_cards[self.cc.opponent_1_id] = BitwiseToCards(
            UnicatesPerPlayer[self.cc.opponent_1_id]
        )
        npc_cards[self.cc.opponent_2_id] = BitwiseToCards(
            UnicatesPerPlayer[self.cc.opponent_2_id]
        )
        ...
        for key, value in CardsToDistributeBitwise.items():
            ...
            CardsToDistribute[key] = self.order_cards_by_strength(
                CardsToDistribute[key]
            )
        if (
            self.player.role == "Off"
        ):  # In this case, opponent is likely to have better cards.
            CardsToDistribute[
                self.cc.partner_id
            ].reverse()  # add a likelihood for each player to have a card !!!

        # Create Generator of Partner Handcards and iterate over them.
        PartnerCardsIterator = combinations(
            CardsToDistribute[self.cc.partner_id],
            int(NumberOfCardsPerPlayer[self.cc.partner_id]),
        )
        skipPartner = 0
        for PartnerCards in PartnerCardsIterator:
            ...
            Opponent1CardsIterator = combinations(
                CardsToDistribute_deepcopy[self.cc.opponent_1_id],
    ...
    def get_roles(self):
        ...
        if self.rootstate["geschoben"]:
            self.roles[0], self.roles[1], self.roles[2], self.roles[3] = (
                self.roles[2],
                self.roles[3],
                self.roles[0],
                self.roles[1],
    ...
    def get_Handsize_and_Unicates_Bitwise(self):
        ...
        players = [
            ...
            copy.deepcopy(placeholder),
            copy.deepcopy(placeholder),
        ]  # players = [partner, opponent_1, opponent_2]
        ...
        NumberOfRandomCardsPerPlayer = {
            player_id: (
                9
                - len(self.cc.played_cards[player_id])
                - np.sum(UnicatesPerPlayer[player_id])
            )
        ...
        for random_dist, unicates in zip(
            CardsToDistributeBitwise.values(), UnicatesPerPlayer.values()
        ):
...
class Node:
    ...
    def alpha_beta_search(self):
        ...
        table_entry = self.transpositionTable.lookup_table(
            self.tree.npc_cards, cards_on_table, get_trumpf(self.state["trumpf"])
        )
        if table_entry != None:
        ...
        self.state["table"] = [
            played_cards_dict(played_card) for played_card in cards_on_table
        ]
        self.tree.player.move_made(self.tree.player.id, self.card, dict(self.state))

        # check if table full:
        if len(cards_on_table) == 4:
            stich = stich_rules[get_trumpf(self.state["trumpf"])](
                played_cards=cards_on_table
            )
        ...
        if isinstance(cards, Card):
            self.children.append(
                Node(copy.deepcopy(self.tree), cards, self.transpositionTable)
            )
        else:
            for card in cards:
                self.children.append(
                    Node(copy.deepcopy(self.tree), card, self.transpositionTable)
                )
    ...
    def add_points(self, team_index, cards):
        ...
        points = count_stich(
            cards,
            get_trumpf(self.state["trumpf"]),
            last=(len(self.state["stiche"]) == 9),
        )
