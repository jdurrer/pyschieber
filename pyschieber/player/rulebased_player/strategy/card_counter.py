...
class CardCounter:
    ...
    def filter_cards_of_same_suit(self, card: Card, predicate: Callable) -> list[Card]:
        ...
        """
        unknown_of_same_suit: list[Card] = list(
            filter(lambda x: x.suit == card.suit, self.unknown_cards())
        )
        return list(filter(predicate, unknown_of_same_suit))

    def filter_suit_cards_from_tuple(
        self, cards: list[tuple[Suit, list[Card]]], suit: Suit
    ) -> list[Card]:
    ...
    def filter_not_dead_cards_of_same_suit(
        self, card: Card, predicate: Callable
    ) -> list[Card]:
    ...
    def had_stich_previously(self, player_id: int) -> bool:
        ...
        """
        return any(
            isinstance(flag, PreviouslyHadStichFlag) for flag in self.flags[player_id]
        )
    ...
    def has_cards_likelihood(
        self, player_id: int, cards: list[Card], state: Status
    ) -> float:
    ...
    def has_card_likelihood(self, player_id: int, card: Card, state: Status) -> float:
        ...
        for flag in self.flags[player_id]:
            failed_to_serve_suit: bool = (
                isinstance(flag, FailedToServeSuitFlag) and card.suit == flag.color
            )
            does_not_have_card: bool = (
                isinstance(flag, DoesntHaveCardFlag) and flag.card == card
            )
    ...
    def get_angezogen_flags(self, player_id: int) -> list[Suit]:
        ...
        return list(
            map(
                lambda y: y.color,
                filter(
                    lambda x: isinstance(x, SuitAngezogenFlag), self.flags[player_id]
                ),
            )
        )
    ...
    def get_doesnt_have_card_flags(self, player_id: int) -> list[Card]:
        ...
        return [
            y.card
            for y in filter(
                lambda x: isinstance(x, DoesntHaveCardFlag), self.flags[player_id]
            )
        ]
    ...
    def get_failed_to_serve_suits_flags(self, player_id: int) -> list[Suit]:
        ...
        return [
            y.color
            for y in filter(
                lambda x: isinstance(x, FailedToServeSuitFlag),
                self.flags[player_id],
            )
        ]
    ...
    def get_had_stich_flags(self, player_id: int) -> list[PreviouslyHadStichFlag]:
        ...
        return list(
            map(
                lambda y: y,
                filter(
                    lambda x: isinstance(x, PreviouslyHadStichFlag),
                    self.flags[player_id],
                ),
            )
        )
    ...
    def get_number_of_trumpf_flag(self, player_id: int) -> NumberOfTrumpfFlag | None:
        ...
        flag_list = list(
            map(
                lambda y: y,
                filter(
                    lambda x: isinstance(x, NumberOfTrumpfFlag), self.flags[player_id]
                ),
            )
        )
