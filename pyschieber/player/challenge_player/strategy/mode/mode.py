...
class Mode:
    ...
    def cards_beating_current_stich(
        self, available_cards: list[Card], card_counter, state: dict
    ) -> list[Card]:
        ...
        if curr_pos != 0:
            for card_played in state["table"]:
                if card_played["player_id"] == card_counter.round_leader(state):
                    current_stich_winner: Card = from_string_to_card(
                        card_played["card"]
                    )
                    break
    ...
    def is_nth_nut(
        self, n: int, card: Card, card_counter
    ) -> bool:  # Not sure what this fun does.
        return (
            len(self.stronger_cards_remaining(card, card_counter)) == 0
        )  # == 0 should probably be changed to == n

    def have_to_serve(
        self, available_suits: list[Suit], round_color: Suit
    ):  # Not really sure if types are correct.
