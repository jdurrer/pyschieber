...
class TrumpfColorMode(Mode):
    ...
    def stronger_cards_remaining(self, card, card_counter):
        ...
        if card.suit == self.suit:
            stronger_cards.extend(
                card_counter.filter_not_dead_cards_of_same_suit(
                    card, lambda x: x.get_trumpf_rank() > card.get_trumpf_rank()
                )
            )
        else:
            for suit_cards in split_cards_by_suit(
                card_counter.remaining_cards(card_counter.dead_cards())
            ):
    ...
    def stronger_cards_unknown(self, card, card_counter):
        ...
        if card.suit == self.suit:
            stronger_cards.extend(
                card_counter.filter_cards_of_same_suit(
                    card, lambda x: x.get_trumpf_rank() > card.get_trumpf_rank()
                )
            )
        else:
        ...
        if color != self.suit:
            if not player_acting_behind:
                ...
                card_counter.has_suit_likelihood(
                    card_counter.opponent_1_id, color, state
                )
                == 1
            ):
                if (
    ...
    def get_tossable_card(self, available_cards, card_counter, state):
        ...
        if (
            len(state["table"]) > 0
            and self.have_to_serve(eligible, round_color)
            and (
                round_color != self.suit
                or not self.has_only_jack_of_trumpf(available_cards)
            )
        ):
            ...
            beatable = (
                card_counter.has_cards_likelihood(
                    card_counter.opponent_1_id,
                    self.cards_beating_current_stich(
                        card_counter.unknown_cards(), card_counter, state
                    ),
    ...
    def get_stich_card(self, cards_by_suit, card_counter, state):
        if len(state["table"]) > 0:
            ...
            tmp_list = [
                x[1]
                for x in cards_by_suit
                if (x[0] == current_stich_color or x[0] == self.suit)
            ]
            current_color_cards = [item for sublist in tmp_list for item in sublist]
            cards = self.cards_beating_current_stich(
                current_color_cards, card_counter, state
            )
            stich_cards = []
            ...
            if len(cards) > 0 and len(state["table"]) < 3:
                for card in cards:
                    ...
                    if len(stronger) != 0:
                        ...
                        if (
                            card_counter.has_cards_likelihood(
                                card_counter.opponent_1_id, stronger, state
                            )
                            == 0
                        ):
                            if (
                                len(state["table"]) > 0
                                or card_counter.has_cards_likelihood(
                                    card_counter.opponent_2_id, stronger, state
                                )
                                == 0
                            ):
    ...
    def get_card_to_play(self, available_cards, card_counter, state, role):
        ...
        opponents_out_of_trumpf = (
            card_counter.has_suit_likelihood(
                card_counter.opponent_1_id, self.suit, state
            )
            == 0
            and card_counter.has_suit_likelihood(
                card_counter.opponent_2_id, self.suit, state
            )
            == 0
        )
        ...
        if current_position == 0:
            if role == "Trumpf" or role == "Off":
                pulling_trumpfs = (
                    not opponents_out_of_trumpf and len(sorted_trumpf_cards) > 0
                )
                if pulling_trumpfs:
